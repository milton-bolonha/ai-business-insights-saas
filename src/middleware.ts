...
const isProtectedRoute = createRouteMatcher([
  // Proteger apenas rotas que requerem autenticação específica
  // Guests podem acessar /admin e operações básicas
  '/api/stripe(.*)', // Pagamentos requerem autenticação
  '/api/usage(.*)', // Estatísticas de uso podem requerer auth
  // Outras rotas como /api/workspace, /api/generate podem ser acessadas por guests
  // mas as operações serão limitadas pela lógica de negócio
]);
    ...
    try {
      ...
      if (!rateLimit.allowed && rateLimit.response) {
        ...
        try {
          ...
          console.warn('[Middleware] Rate limit exceeded (audit logging unavailable in Edge Runtime):', req.nextUrl.pathname);
        }
