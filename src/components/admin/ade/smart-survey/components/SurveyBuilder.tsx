...
export function SurveyBuilder({ initialSurvey, onSave, onCancel }: Props) {
  ...
  function addQuestion(type: Question["type"]) {
    const newQ: Question = { 
      id: `q_${generateId()}`, type, label: t("admin.smartSurvey.newQuestion"), weight: 2,
      ...(type.includes("choice") ? { options: [t("admin.smartSurvey.option1"), t("admin.smartSurvey.option2")] } : {}),
      ...(type === "scale_0_10" ? { minLabel: t("admin.smartSurvey.disagree"), maxLabel: t("admin.smartSurvey.agree") } : {})
    };
    ...
    <div className="fixed inset-0 z-[100] flex flex-col bg-neutral-50 animate-in fade-in duration-200">
      <div className="bg-white border-b border-neutral-200 px-6 py-4 flex items-center justify-between shadow-sm z-10">
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="text-[10px] uppercase font-bold text-neutral-400 tracking-wider">{t("admin.smartSurvey.studioTitle")}</div>
            <h2 className="font-serif text-lg font-semibold text-neutral-900">{survey.title || t("admin.smartSurvey.newForm")}</h2>
          </div>
        </div>
        <button onClick={() => onSave(survey)} disabled={!survey.title.trim()} className="bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-2.5 rounded-lg text-xs font-bold flex items-center gap-2 shadow-md transition-all disabled:opacity-50 cursor-pointer">
          <Save size={14} /> {t("admin.smartSurvey.saveForm")}
        </button>
      </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-80 bg-white border-r border-neutral-200 flex flex-col z-0 shadow-sm shrink-0">
          <div className="flex border-b border-neutral-100">
            <button onClick={() => setActiveTab("meta")} className={`flex-1 py-3.5 text-[11px] font-bold uppercase tracking-wider cursor-pointer transition-colors ${activeTab === 'meta' ? 'text-neutral-900 border-b-2 border-neutral-900 bg-neutral-50/50' : 'text-neutral-400 hover:bg-neutral-50 hover:text-neutral-600'}`}>{t("admin.smartSurvey.basicInfo")}</button>
            <button onClick={() => setActiveTab("questions")} className={`flex-1 py-3.5 text-[11px] font-bold uppercase tracking-wider cursor-pointer transition-colors ${activeTab === 'questions' ? 'text-neutral-900 border-b-2 border-neutral-900 bg-neutral-50/50' : 'text-neutral-400 hover:bg-neutral-50 hover:text-neutral-600'}`}>{t("admin.smartSurvey.questions")} ({(survey.questions || []).length})</button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-5">
            {activeTab === "meta" && (
              <div className="space-y-6">
                <div>
                  <label className="block text-[10px] font-bold text-neutral-500 uppercase mb-1.5">{t("admin.smartSurvey.formTitle")}</label>
                  <input type="text" value={survey.title} onChange={e => updateSurvey("title", e.target.value)} placeholder={t("admin.smartSurvey.formTitlePlaceholder")} className="w-full border border-neutral-300 rounded-md p-2.5 text-sm bg-neutral-50 focus:bg-white focus:outline-none focus:border-emerald-500 transition-colors" />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-neutral-500 uppercase mb-1.5">{t("admin.smartSurvey.descObjective")}</label>
                ...
                <div>
                  <label className="block text-[10px] font-bold text-neutral-500 uppercase mb-2 flex items-center gap-1.5"><Palette size={12}/> {t("admin.smartSurvey.visualIdentity")}</label>
                      ...
                      <div className="truncate pr-2 w-full">
                        ...
                        <div className="flex items-center justify-between w-full">
                          ...
                          <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded uppercase ${q.weight === 3 ? 'bg-red-100 text-red-700' : q.weight === 2 ? 'bg-amber-100 text-amber-700' : 'bg-emerald-100 text-emerald-700'}`}>
                            {t("admin.smartSurvey.weight")} {q.weight || 1}
                          </span>
                        </div>
                      </div>
                    </div>
          ...
          <div className="max-w-3xl mx-auto">
            {!editingQ ? (
              <div className="flex flex-col items-center justify-center h-full text-center mt-32 opacity-60">
                ...
                <h3 className="text-xl font-serif font-bold text-neutral-600">{t("admin.smartSurvey.topologyTitle")}</h3>
                <p className="text-sm text-neutral-500 max-w-sm mt-2 leading-relaxed">{t("admin.smartSurvey.topologyDesc")}</p>
              </div>
            ) : (
              (() => {
                ...
                return (
                  <div className="bg-white rounded-xl shadow-xl border border-neutral-200 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                    <div className="bg-neutral-900 px-6 py-4 flex items-center justify-between">
                      <span className="text-white text-xs font-bold uppercase tracking-wider flex items-center gap-2"><Edit3 size={16}/> {t("admin.smartSurvey.paramConfig")}</span>
                      <button onClick={() => deleteQuestion(q.id)} className="text-red-400 hover:text-white hover:bg-red-600 p-1.5 bg-neutral-800 rounded transition-colors cursor-pointer" title={t("admin.smartSurvey.deleteQuestion")}><Trash2 size={16}/></button>
                    </div>
                    
                    <div className="p-6 md:p-8 space-y-8">
                      <div>
                        <label className="block text-[11px] font-bold text-neutral-500 uppercase mb-2">{t("admin.smartSurvey.questionLabel")}</label>
                      ...
                      <div className="bg-amber-50 border border-amber-200 rounded-xl p-5 shadow-sm">
                        <div className="flex items-start justify-between mb-4">
                          <div>
                            <label className="flex items-center gap-1.5 text-xs font-bold text-amber-800 uppercase tracking-wider"><ShieldAlert size={16}/> {t("admin.smartSurvey.structuralWeight")}</label>
                            <p className="text-[11px] text-amber-700 mt-1.5 max-w-lg leading-relaxed" dangerouslySetInnerHTML={{ __html: t("admin.smartSurvey.weightDesc") }} />
                          </div>
                        </div>
                        <div className="flex bg-white rounded-lg border border-amber-200 p-1 shadow-sm max-w-md">
                          {[
                            { val: 1, label: t("admin.smartSurvey.weightLow"), color: "#10b981", bg: "#ecfdf5" },
                            { val: 2, label: t("admin.smartSurvey.weightMedium"), color: "#f59e0b", bg: "#fffbeb" },
                            { val: 3, label: t("admin.smartSurvey.weightHigh"), color: "#ef4444", bg: "#fef2f2" }
                          ].map(opt => (
                            <button
                              key={opt.val}
                      ...
                      {q.type.includes("choice") && (
                        <div>
                          <label className="block text-[11px] font-bold text-neutral-500 uppercase mb-3">{t("admin.smartSurvey.answerOptions")}</label>
                          <div className="space-y-3">
                            ...
                            <button onClick={() => updateQuestion(q.id, { options: [...(q.options || []), `${t("admin.smartSurvey.newOption")} ${(q.options || []).length + 1}`] })} className="text-xs font-bold text-emerald-600 hover:text-emerald-800 mt-3 inline-flex items-center gap-1.5 cursor-pointer bg-emerald-50 px-3 py-2 rounded-md transition-colors"><Plus size={14}/> {t("admin.smartSurvey.addOption")}</button>
                          </div>
                        </div>
                      )}
                      ...
                      {q.type === "scale_0_10" && (
                        <div className="grid grid-cols-2 gap-5">
                          <div className="bg-neutral-50 p-4 rounded-xl border border-neutral-100">
                            <label className="block text-[11px] font-bold text-neutral-500 uppercase mb-2">{t("admin.smartSurvey.minLabel")}</label>
                            <input type="text" value={q.minLabel || ""} onChange={e => updateQuestion(q.id, { minLabel: e.target.value })} placeholder={t("admin.smartSurvey.minPlaceholder")} className="w-full border border-neutral-300 rounded-lg p-2.5 text-sm focus:outline-none focus:border-emerald-500" />
                          </div>
                          <div className="bg-neutral-50 p-4 rounded-xl border border-neutral-100">
                            <label className="block text-[11px] font-bold text-neutral-500 uppercase mb-2">{t("admin.smartSurvey.maxLabel")}</label>
                            <input type="text" value={q.maxLabel || ""} onChange={e => updateQuestion(q.id, { maxLabel: e.target.value })} placeholder={t("admin.smartSurvey.maxPlaceholder")} className="w-full border border-neutral-300 rounded-lg p-2.5 text-sm focus:outline-none focus:border-emerald-500" />
                          </div>
                        </div>
                      )}
                    </div>
