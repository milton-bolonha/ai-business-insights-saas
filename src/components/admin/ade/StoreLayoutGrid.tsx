...
export function StoreLayoutGrid({ tiles, onSaveLayout, appearance, onExport }: StoreLayoutGridProps) {
    ...
    const [newSector, setNewSector] = useState<{
        ...
        cols: number | string;
        orientation: "horizontal" | "vertical";
    }>({ name: '', rows: 4, cols: 6, orientation: 'horizontal' });
        ...
        <div className="flex flex-col gap-3 shrink-0 mb-4 bg-white p-3 rounded-2xl shadow-sm border border-slate-200">
            <div className="flex justify-between items-center px-2 pt-1">
                <div className="flex items-center gap-2">
                    ...
                    <h2 className="font-bold text-slate-800 text-sm">Visão do Armazém</h2>
                </div>
                <div className="flex items-center gap-2">
                    <button 
                        onClick={() => setIsEditorMode(!isEditorMode)} 
                        className={cn("p-1.5 rounded-lg border transition-all", isEditorMode ? "bg-slate-800 text-white border-slate-800" : "bg-slate-50 border-slate-200 text-slate-500")}
                    ...
                    <button 
                        ...
                        className="p-1.5 bg-slate-50 border border-slate-200 text-slate-500 hover:text-blue-600 rounded-lg shadow-sm transition-all"
                    >
        ...
        return (
            <div className="w-full flex justify-center flex-1 min-h-0 relative animate-fade-in">
                <div className="relative flex items-stretch group transition-all duration-300 w-full">
                    ...
                    {/* Grid Canvas */}
                    <div className={cn(
                        "bg-white border border-slate-200 rounded-2xl p-4 shadow-sm w-full max-w-full flex flex-col",
                        isCompactView ? 'overflow-hidden' : 'overflow-auto custom-scrollbar'
                    )}>
                        <div 
                            className={cn(
                                "grid",
                                ...
                                return (
                                    <button 
                                        ...
                                        className={cn(
                                            ...
                                            isSelected ? 'ring-4 ring-blue-500/30 z-10 border-blue-500 scale-105' : 'hover:scale-[1.02]'
                                        )}
                                    >
                                        <span className={cn(
    ...
    return (
        <div className="flex flex-col flex-1 bg-slate-50/50 p-2 sm:p-4 min-h-0 h-full overflow-hidden">
            {renderHeader()}
                                    ...
                                    {sections.map(sec => (
                                        <div key={sec.id} className="bg-white border border-slate-200 p-4 rounded-2xl flex justify-between items-center group hover:border-blue-300 transition-all">
                                            ...
                                            <div className="flex gap-1">
                                                <button onClick={() => { setEditingSectorId(sec.id); setNewSector({ name: sec.id, rows: sec.rows, cols: sec.cols, orientation: sec.orientation || 'horizontal' }); }} className="p-2 text-slate-400 hover:text-blue-600 transition-all"><Edit size={16}/></button>
                                                <button onClick={() => {
