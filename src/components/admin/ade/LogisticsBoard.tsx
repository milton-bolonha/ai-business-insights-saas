...
export function LogisticsBoard({ tiles, appearance, onUpdateTile, onAddNote, onOpenOrderModal }: LogisticsBoardProps) {
    ...
    return (
        <div className="space-y-6 p-4 sm:p-8">
            ...
            <div className="grid grid-cols-2 md:flex md:overflow-x-auto hide-scrollbar gap-2 bg-gray-50 p-2 rounded-3xl border border-gray-100">
                {[
                    ...
                    <button
                        ...
                        className={cn(
                            "flex-1 min-w-[90px] px-4 py-2.5 rounded-2xl text-xs font-black uppercase tracking-widest transition-all cursor-pointer text-center",
                            activeRole === role.id
                                ? "bg-gray-900 text-white shadow-xl"
                                : "text-gray-400 hover:text-gray-900 bg-white md:bg-transparent border border-gray-100 md:border-transparent"
                        )}
                    >
                        {role.label}
                    </button>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center w-full sm:w-auto gap-3">
                    ...
                    <button
                        ...
                        className={cn(
                            "flex items-center justify-center gap-2 px-5 py-3 rounded-2xl text-sm font-black uppercase tracking-widest transition-all border-2 cursor-pointer w-full sm:w-auto",
            ...
            {/* Kanban Columns Selector (Mobile Only) Dropdown */}
            <div className="md:hidden w-full px-1">
                <div className="relative">
                    <select
                        value={activeMobileColumn}
                        onChange={(e) => setActiveMobileColumn(Number(e.target.value))}
                ...
                {[
                    ...
                    return (
                        <div
                            key={colIdx}
                            className={cn(
                                "bg-gray-50/50 border border-gray-100 rounded-[2rem] p-4 flex flex-col gap-4 min-h-[400px]",
                                activeMobileColumn !== colIdx && "hidden md:flex"
                            )}
                        >
